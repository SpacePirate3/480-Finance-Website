...
class TestEnvironment(unittest.TestCase):
    ...
    #             self.assertEqual(list(resp.json().keys()),['name','symbol'])
    ...
    def test_api_stockOverview(self):
        ...
        for i, stock_symbol in zip(range(len(self.stocks)),self.stocks):
            with self.subTest(i=i):
                resp = requests.get(self.api + f"stock/overview/{stock_symbol}/")

                # Check HTTP status code
                self.assertEqual(resp.status_code, 200, f'Response code for stock {stock_symbol} was {resp.status_code}')

                # Check the structure of the response
                expected_keys = ['model', 'pk', 'fields']
                self.assertEqual(list(resp.json().keys()), expected_keys,f'Keys are unexpected in stock {stock_symbol}')
                ...
                self.assertSetEqual(set(fields.keys()), expected_fields,f'fields for stock {stock_symbol} are incorrect. ')
                ...
                self.assertIsInstance(fields["market_capitalization"], int, f'Market capitilazition in stock {stock_symbol} should be an int')
                floatKeys = {"pe_ratio", "peg_ratio", "book_value", "dividend_yield", "eps",
                ...
                #these keys may be null as a certain company may not use dividends
                maybeNullFloatKeys={'dividend_per_share','dividend_yield','dividend_date','ex_dividend_date'}
                # Check that each value in the fields dictionary is not blank
                for key, value in fields.items():
                    self.assertNotEqual(value, '', f"Value for '{key}' should not be blank.")
                    ...
                    if key in floatKeys:
                        try:
                            ...
                            if not(key in maybeNullFloatKeys): 
                                self.fail(f"Value for '{key}' in stock {stock_symbol} should be convertible to float.")
